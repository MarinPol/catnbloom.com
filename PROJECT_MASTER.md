...
| 2026-08-02 | _layouts/sitemap.html (new) + sitemap.md (new) + _includes/breadcrumbs.html + assets/css/style.css | Создан HTML Sitemap для людей: новый layout на базе паттерна studio-notes.html (без hero-картинки, списки вместо карточек), новая страница /sitemap/, динамические Liquid-циклы по site.pages (layout:collection), site.cottagecore, site.witchy, site.smart, site.albums, site.studio_notes — по аналогии с header.html. Добавлен кейс layout:sitemap в breadcrumbs.html. Новый CSS-блок "SITEMAP PAGE" добавлен в конец style.css без правок существующих модулей, используются существующие design-tokens. _config.yml не менялся — layout указан напрямую в front matter страницы | Реализация Шага 4 плана Sitemap. Проверено вживую на catnbloom.com/sitemap/ — рендер и иконки Lucide корректны, замечаний по оформлению нет |
| 2026-08-02 | _includes/footer.html | Добавлена иконка Sitemap в .footer-social-row (после RSS-иконки), ссылка на /sitemap/, используется существующий класс .footer-social-icon без новых стилей | Устранение "страницы-сироты" — HTML sitemap теперь доступна из глобальной навигации на всех страницах сайта |
...
| HTML Sitemap: лимит SKU-ссылок | ⏳ | Когда в SECTION-коллекции (cottagecore/witchy) станет >40 товаров — заменить полный список SKU на sitemap.html ссылкой только на саму секцию, без перечисления MATERIAL-страниц. Полный список URL для роботов продолжает отдавать /sitemap.xml (лимитов не имеет). Цель: не превращать HTML sitemap в неструктурированный список ссылок |
